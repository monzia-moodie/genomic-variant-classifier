...
class InterpretabilityAgent(BaseAgent):
    ...
    def run(self) -> AgentResult:
        ...
        cb_result: dict[str, Any] = {"analyzed": False, "ranking": [], "error": None}
        ...
        # CatBoost — uses native ShapValues API via Pool, not shap.TreeExplainer
        cb_result = self._analyze_model(
            model_path=ensemble_dir / "catboost_model.cbm",
            X_val=X_val,
            feature_names=feature_names,
            model_type="catboost",
            previous_ranking=prev_rankings.get("catboost"),
        )
        if cb_result.get("analyzed"):
            results["importance_anomalies"] += cb_result.get("importance_anomalies", [])
            results["biological_anomalies"] += cb_result.get("biological_anomalies", [])
        if cb_result.get("error"):
            results["errors"].append(f"CatBoost SHAP: {cb_result['error']}")
        ...
        if resnet_result.get("error"):
            results["errors"].append(f"ResNet GradCAM: {resnet_result['error']}")

        # ---- 5. Top features + UniProt context --------------------------
        ...
        self._update_state(xgb_result, lgb_result, cb_result, results)

        success = (results["xgb_analyzed"] or results["lgb_analyzed"]) and not results[
            "errors"
        ]
        return AgentResult(
            success=success,
    ...
    def _load_val_data(self) -> tuple[np.ndarray | None, np.ndarray | None, list[str]]:
        ...
        try:
            ...
            self.log.info(
                "Validation data loaded: %d samples, %d features.",
                X.shape[0],
                X.shape[1],
            )
    ...
    def _analyze_model(
        ...
        ranking = rank_features(shap_data["mean_abs_shap"], feature_names, SHAP_TOP_K)
    ...
    def _analyze_resnet(self, resnet_dir: Path) -> dict[str, Any]:
        ...
        try:
            ...
            for class_idx in range(RESNET_NUM_CLASSES):
                class_indices = [
                    i for i, (_, lbl) in enumerate(dataset.samples) if lbl == class_idx
                ][:n_samples_per_class]
    ...
    def _save_heatmap(heatmap: np.ndarray, path: Path, title: str = "") -> None:
        try:
            ...
            fig.tight_layout()
            fig.savefig(
                path, dpi=90, bbox_inches="tight", facecolor=fig.get_facecolor()
            )
            plt.close(fig)
        except Exception as exc:
    ...
    def _flag_anomalies(self, results: dict) -> None:
        # Stability flags
        for model_key, stab_key in [
            ("xgb_analyzed", "stability_xgb"),
    ...
    def _update_state(self, xgb_result: dict, lgb_result: dict, cb_result: dict, results: dict) -> None:
        ...
        if cb_result.get("ranking"):
            rankings["catboost"] = cb_result["ranking"]
